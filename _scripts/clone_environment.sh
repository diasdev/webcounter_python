...
if [ -z "$ENV_NAME" ]
then
    read -p $'\033[0;33mNew Environment name: \033[00m' ENV_NAME;
fi
...
curl -X 'POST' \
  "https://api.environments.bunnyshell.com/api/environments/$environment_number/clone" \
  -H 'accept: application/ld+json' \
  -H "X-Auth-Token: $BUNNYSHELL_API_TOKEN" \
  -H 'Content-Type: application/json' \
  -d '{
